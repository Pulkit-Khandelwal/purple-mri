"""
Read the vtk files and then replace zero thickness with NaN
and then save the vtk files in the correct vtk format and binarized
making them ready for glm analyses
"""
...
###### Main script
dir_vtk_files_input=''
dir_vtk_files_output=''

subjects = [f for f in os.listdir(dir_vtk_files_input) if f.endswith('.vtk')]
...
for fwhm in fwhm_list:
    for surf_type in surf_type_list:
        ...
        for subj in subjects:
            ...
            save_vtk(pd, dir_vtk_files_output + subj, binary=True)


"""
######## This is for QC and can be used separately for sanity check::::
######## i.e., are there any cases with zero thickness in the template space
######## or if the image/segm were not correctly flipped as a "right" hemis and that is why we see a zero thikcness
