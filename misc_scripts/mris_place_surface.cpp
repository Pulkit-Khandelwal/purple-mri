...
double mid_gray = 142; // PK: was 67.5

int max_pial_averages = 48 ; //PK was 16
int min_pial_averages = 2 ;
int max_white_averages = 4 ;
int min_white_averages = 0 ;
float pial_sigma = 1.0f ; //PK was 2.0f
...
const char *involname="aseg.presurf_100.mgz", *segvolname="aseg.presurf_100.mgz",*wmvolname="wm.mgz",*aparcname="aparc";
...
int main(int argc, char **argv) 
{
  ...
  if(DoIntensityProc){
    ...
    printf("WE ARE USING THE INTENSITIES AS PROVIDED BY PULKIT KHANDELWAL\n");
    involCBV = invol;
    involPS  = invol;
  }
 ...
 fflush(stdout);
  ...
  for (i = 0 ;  n_averages >= n_min_averages ; n_averages /= 2, current_sigma /= 2, i++) {
    ...
    // MMS runs	MRISsetCroppedToZero(mris) with T2/FLAIR, but not clear it does anythin
    if(!n_averages)
      break ; 
...
static int parse_commandline(int argc, char **argv) {
  ...
  while (nargc > 0) {
	...
	exit(1);
      ...
      fitparms.niterations = 200; //PK: was 10
