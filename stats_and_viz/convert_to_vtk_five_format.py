...
subjects = [f for f in os.listdir(dir_roi_files)]
print("the subjects: ", subjects, len(subjects))
surface='pial'

for subj in subjects:
    print(subj)
    files_in_subject = [f for f in os.listdir(dir_roi_files+subj) if f.endswith('.fsaverage.' + surface + '.vtk')]
    print("total subjects_in_roi_files: ", len(files_in_subject))
    for curr_file in files_in_subject:
        print(curr_file[:-4])
        md_src = load_mesh(dir_roi_files + subj + '/' + curr_file)
        pd = vtk_make_pd(md_src['v'], md_src['f'])
        vtk_set_cell_array(pd, 'dots', md_src['curv'])
        file_path = dir_roi_files + subj + '/' + curr_file[:-4] + '_v5.vtk'
        print(file_path)
